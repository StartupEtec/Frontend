...
describe('OtpVerificationScreen', () => {
  ...
  it('renders title, subtitle with masked contact, all 6 digit inputs, and timer', () => {
    const { getByText, getByTestId } = render(<OtpVerificationScreen {...DEFAULT_PROPS} />);
  ...
  it('renders back button and navigates back when pressed', () => {
    const { getByTestId } = render(<OtpVerificationScreen {...DEFAULT_PROPS} />);
  ...
  it('enables verify button only when all 6 digits are filled', () => {
    ...
    expect(getByTestId('btn-otp-verify').props.accessibilityState?.disabled).toBe(true);

    fireEvent.changeText(getByTestId('otp-input-5'), '6');
    expect(getByTestId('btn-otp-verify').props.accessibilityState?.disabled).toBe(false);
  });
  ...
  it('calls verifyOtp and navigates on successful verification', async () => {
    (authService.verifyOtp as jest.Mock).mockResolvedValueOnce({ success: true, message: 'OK' });
    const { getByTestId } = render(<OtpVerificationScreen {...DEFAULT_PROPS} />);
  ...
  it('shows error message on invalid code (400) and decrements attempts', async () => {
    ...
    const { getByTestId, getByText } = render(<OtpVerificationScreen {...DEFAULT_PROPS} />);
    for (let i = 0; i < 6; i++) {
  ...
  it('locks the screen after 5 failed attempts', async () => {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      fireEvent.press(getByTestId('btn-otp-verify'));
      await waitFor(() => expect(authService.verifyOtp).toHaveBeenCalledTimes(attempt + 1));
    }
  ...
  it('shows OTP expiry error when the 10-minute countdown reaches zero', () => {
    const { getAllByText, getByTestId } = render(<OtpVerificationScreen {...DEFAULT_PROPS} />);
    ...
    expect(getAllByText(/[Cc]ódigo expirado|El código ha expirado/).length).toBeGreaterThanOrEqual(1);
  ...
  it('calls resendOtp, resets timer, and re-enables input after resend', async () => {
    (authService.resendOtp as jest.Mock).mockResolvedValueOnce({ success: true, message: 'Enviado' });
    ...
    await waitFor(() => {
      expect(authService.resendOtp).toHaveBeenCalledWith({ contact: DEFAULT_PROPS.contact });
    });
  });
